export default class Circle {
  ...
  update(canvasW, canvasH) {
    ...
    if (this.x < this.R || this.x > canvasW - this.R) this.dX = -this.dX;

    // check Canvas horizontal collisions
    if (this.y < this.R || this.y > canvasH - this.R) this.dY = -this.dY;
  ...
  circleCollisions() {
    for (let i = 0; i < Circle.allCircles.length; i++) {
      for (let j = i + 1; j < Circle.allCircles.length; j++) {
        checkCollision(Circle.allCircles[i], Circle.allCircles[j]);
      }
    }
  }
}
